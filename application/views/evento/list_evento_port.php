...
<div id="page-wrapper">
	...
	<div class="row">
		<div class="col-lg-12">
<?php
//var_dump($usuario_port);
//var_dump($evento_usuario);
if ($usuario_port){
	$usuario_port_row = $usuario_port->row();

	$usuario_port_form = array(
		NOMBRE => array(
			'class' => 'form-control',
			'name' => NOMBRE,
			'value' => $usuario_port_row->nombre,
			'readonly' => TRUE
		),
		APELLIDO => array(
			'class' => 'form-control',
			'name' => APELLIDO,
			'value' => $usuario_port_row->apellido,
			'readonly' => TRUE
		),
		TELEFONO => array(
			'class' => 'form-control',
			'name' => TELEFONO,
			'value' => $usuario_port_row->telefono,
			'readonly' => TRUE
		),
		DIRECCION => array(
			'class' => 'form-control',
			'name' => DIRECCION,
			'value' => $usuario_port_row->direccion,
			'readonly' => TRUE
		),
		EMAIL => array(
			'class' => 'form-control',
			'name' => EMAIL,
			'value' => $usuario_port_row->email,
			'readonly' => TRUE
		),
		FACEBOOK => array(
			'class' => 'form-control',
			'name' => FACEBOOK,
			'value' => $usuario_port_row->facebook,
			'readonly' => TRUE
		),
		TWITTER => array(
			'class' => 'form-control',
			'name' => TWITTER,
			'value' => $usuario_port_row->twitter,
			'readonly' => TRUE
		),
		URL_FOTO => array(
			'class' => 'form-control',
			'name' => URL_FOTO,
			'value' => $usuario_port_row->url_foto,
			'readonly' => TRUE
		)
	);
}
						...
						<div class="col-lg-12">
							<div class="well">
								<div class="row">
									<div class="col-lg-12">
										<h4>
											<?= $usuario_port_form[NOMBRE]['value'].' '.$usuario_port_form[APELLIDO]['value'];?>
										</h4>
									</div>
								</div>
								<div class="row">
									...
									<div class="col-lg-4">
										<div class="form-group">
											...
											<?= form_input($usuario_port_form[DIRECCION]); ?>
										</div>
										<div class="form-group">
											<?= form_label($usuario_rol_rules[TWITTER]['label'],$usuario_rol_rules[TWITTER]['field']); ?>
											<?= form_input($usuario_port_form[TWITTER]); ?>
										</div>
									</div>
									<!-- /.col-lg-4 (nested) -->
									<div class="col-lg-4">
...
?>
								<tr>
									<td colspan="5"><center><h3>Sin Eventos asociados.</h3></center></td>
								</tr>
<?php
	}
?>
